...
import httpx
from any_agent.validation.a2a_message_validator import A2AMessageValidator, A2AValidationResult
...
class TestA2AMessageValidator:
    ...
    async def test_successful_protocol_tests(self):
        ...
        async def mock_agent_card_discovery(base_url, httpx_client):
            tester.validation_results.append(A2AValidationResult(
    ...
    async def test_failed_protocol_tests(self):
        ...
        failed_results = [validation for validation in result["validations"] if not validation["success"]]
    ...
    async def test_agent_card_discovery(self):
        ...
        with patch('any_agent.validation.a2a_message_validator.A2ACardResolver', return_value=mock_resolver):
            ...
            await tester._validate_agent_card_discovery("http://localhost:8080", mock_client)
        ...
        with patch('any_agent.validation.a2a_message_validator.A2ACardResolver', return_value=mock_resolver):
            with patch('any_agent.validation.a2a_message_validator.ClientFactory', return_value=mock_factory):
                with patch('any_agent.validation.a2a_message_validator.ClientConfig'):
                    ...
                    await tester._validate_client_connection("http://localhost:8080", httpx_client)
            ...
            with patch('any_agent.validation.a2a_message_validator.ClientFactory', return_value=mock_factory):
                with patch('any_agent.validation.a2a_message_validator.ClientConfig'):
                    with patch('any_agent.validation.a2a_message_validator.create_text_message_object') as mock_create_msg:
                        ...
                        httpx_client = MagicMock()
                        await tester._validate_basic_message_exchange("http://localhost:8080", httpx_client)
