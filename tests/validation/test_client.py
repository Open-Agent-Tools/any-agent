...
class TestA2AValidationClient:
    ...
    def test_init_with_auth_bearer(self):
        ...
        config = A2AValidationConfig(
            endpoint="http://localhost:8080", auth_token="secret123", auth_type="bearer"
        )
        client = A2AValidationClient(config)
    ...
    def test_init_with_auth_api_key(self):
        ...
        config = A2AValidationConfig(
            endpoint="http://localhost:8080", auth_token="api123", auth_type="api_key"
        )
        client = A2AValidationClient(config)
    ...
    async def test_get_agent_card(self):
        ...
        agent_card = {
            ...
            "version": "1.0.0",
            "capabilities": [
                {"method": "a2a.ping", "description": "Health check method"}
            ],
        }
    ...
    async def test_batch_call(self):
        ...
        with patch.object(
            client, "call_method", side_effect=[ping_result, test_result]
        ):
