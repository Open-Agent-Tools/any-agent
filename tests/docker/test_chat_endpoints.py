...
class TestChatEndpointsGeneration:
    ...
    def test_generate_chat_endpoints_generic_includes_cleanup(self, generator):
        ...
        assert 'app.post("/chat/create-session")(create_chat_session_endpoint)' in endpoints_code
        assert 'app.post("/chat/send-message")(send_chat_message_endpoint)' in endpoints_code
        assert 'app.post("/chat/cleanup-session")(cleanup_chat_session_endpoint)' in endpoints_code
    ...
    def test_adk_entrypoint_includes_chat_endpoints(self, generator, mock_metadata):
        ...
        entrypoint_code = generator._generate_adk_entrypoint(agent_path, mock_metadata, add_ui=True)
    ...
    def test_strands_entrypoint_includes_chat_endpoints(self, generator, mock_metadata):
        ...
        agent_path = Path("/test/agent")
        entrypoint_code = generator._generate_strands_entrypoint(agent_path, strands_metadata, add_ui=True)
    ...
    def test_all_frameworks_support_cleanup(self, generator):
        ...
        for framework in frameworks:
            ...
            assert "cleanup_chat_session_endpoint" in endpoints_code, f"Framework {framework} missing cleanup endpoint"
    ...
    def test_http_methods_consistency(self, generator):
        ...
        expected_paths = ["/chat/create-session", "/chat/send-message", "/chat/cleanup-session", "/chat/cancel-task"]
    ...
    def test_agent_url_default_handling(self, generator):
        ...
        assert "agent_url = request_body.get('agent_url', f'http://localhost:{os.getenv(\"AGENT_PORT\")}'" in endpoints_code
...
class TestChatEndpointIntegration:
    ...
    def test_complete_adk_entrypoint_with_cleanup(self, generator):
        ...
        entrypoint_code = generator._generate_adk_entrypoint(Path("/test"), metadata, add_ui=True)
        
        # Should include imports needed for cleanup
        assert "from any_agent.api.chat_handler import A2AChatHandler" in entrypoint_code
