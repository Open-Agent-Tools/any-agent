...
class TestUnifiedDockerfileGeneratorComprehensive:
    ...
    def test_generate_dockerfile_content_structure(self, tmp_path):
        ...
        uv_install_found = any("pip install --no-cache-dir uv" in line for line in lines)
    ...
    def test_generate_entrypoint_google_adk(self, tmp_path):
        ...
        entrypoint_content = generator.generate_entrypoint(agent_path, metadata, add_ui=False)
        
        # Should delegate to _generate_adk_entrypoint
        assert isinstance(entrypoint_content, str)
        assert len(entrypoint_content) > 0

    @patch('any_agent.docker.docker_generator.UnifiedDockerfileGenerator._generate_adk_entrypoint')
    ...
    def test_framework_configs_completeness(self):
        ...
        required_keys = ["default_port", "env_vars", "dependencies", "entrypoint_script", "server_type"]
        ...
        for framework, config in generator.framework_configs.items():
            ...
            assert config["server_type"] in ["google_adk_a2a", "strands_a2a", "generic_a2a"]
    ...
    def test_port_assignments_unique(self):
        ...
        assert len(ports) == len(set(ports)), "Duplicate ports found in framework configs"
